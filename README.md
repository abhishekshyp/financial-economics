# Financial Economics Tutorials — Course Handbook

Welcome. This handbook explains how the tutorials run, how you are graded, the tools you will need, and a curated set of readings, courses, and resources to go deeper. Each week's module (slides, tools, and assignment) lives in its own folder; this page is the map.

Ready, set, go! https://abhishekshyp.github.io/financial-economics

---

## Tutorial structure — what to expect

Every tutorial is designed to maximize learning through active engagement, and follows the same four-part rhythm:

1. **Play** — a game or interactive tool to build intuition *before* the theory.
2. **Lecture slides** — the concepts, models, and frameworks.
3. **Questions** — in-session checks to consolidate understanding.
4. **Assignment** — a graded, take-home task that applies the tutorial.

There are **10 tutorials**, and **each one carries a graded assignment**.

---

## Course details

| Week | Tutorial |
|---|---|
| **1** | **Build Your Financial Life** — *where does a whole lifetime of income actually go?* |
| **2** | **What's Your Risk Appetite?** — *how much uncertainty can you stomach?* |
| **3–4** | **Build Your Personal Portfolio** — *from a blank slate to a portfolio that's genuinely yours* |
| **5** | **Asset Accumulation & the Rebalancing Problem** — *your winners grow and your mix drifts — when do you correct course?* |
| **6** | **Can You Beat the Market?** — *skill or luck? put the efficient-market hypothesis to the test* |
| **7** | **Measuring Portfolio Performance** — *returns are easy; risk-adjusted returns tell the real story* |
| **8** | **Other Ways to Hedge Risk** — *diversification, options, insurance — tools beyond just holding cash* |
| **9** | **Active Investing: Direct Equity, DIY** — *think you can pick stocks? let's check your smartness* |
| **10** | **Lazy Investing** — *do less, win more? the case for going passive* |

---

## Assignments & grading

| Component | Marks |
|---|---|
| Submitting an assignment | **2** per tutorial |
| Placing in the **top 3** submissions | **+2** per tutorial (bonus) |

- Submit all ten assignments → **2 × 10 = 20 marks**.
- Submit all ten *and* finish in the top 3 every time → **(2 + 2) × 10 = 40 marks**.

In short: **20 marks** are within reach through consistent submission, and the remaining **20** reward standout work. Submission points are the floor; the bonus is the ceiling.

---

## Tools you'll need

- **Python** — analysis, simulation, and modelling.
- **Excel** — spreadsheet models and quick calculations.
- **AI tools** — for exploration, coding assistance, and research.

---

## Suggested readings

- **Investments** — Bodie, Kane & Marcus
- **The Economics of Money, Banking and Financial Markets** — Mishkin

> Specific chapters, papers, and additional readings will be shared as and when required.

---

## Suggested courses

- [**Finance Theory I** — MIT OCW 15.401](https://ocw.mit.edu/courses/15-401-finance-theory-i-fall-2008/pages/syllabus/)
- [**Personal Finance Fundamentals** — SoFi (Coursera)](https://www.coursera.org/specializations/personal-finance-fundamentals)
- [**Financial Markets** — Yale (Coursera)](https://www.coursera.org/learn/financial-markets-global)
- [**AI for Investments** — IIT Roorkee (NPTEL)](https://onlinecourses.nptel.ac.in/e-learning/preview/noc23_mg63)
- [**Mathematics with Applications in Finance** — MIT OCW 18.S096](https://ocw.mit.edu/courses/18-s096-topics-in-mathematics-with-applications-in-finance-fall-2013/)
- [**Financial Engineering** — Columbia (Coursera)](https://www.coursera.org/specializations/financialengineering)

---

## Other useful resources

**Markets, data & India context**
- [NSE](https://www.nseindia.com) · [BSE](https://www.bseindia.com) · [NiftyIndices](https://www.niftyindices.com) — exchange and index data
- **NSE Market Pulse** — NSE's periodic review of Indian markets
- [Screener.in](https://www.screener.in) — company financials and stock screening
- **Blume — Indus Valley Annual Report 2025** — a data-rich read on the Indian economy and startups
- [Zerodha Varsity](https://zerodha.com/varsity/) — self-paced markets education
- [The Ken](https://the-ken.com/) — long-form business journalism

**Letters, memos & archives**
- [Howard Marks — Memos](https://www.oaktreecapital.com/insights/memos)
- [Berkshire Hathaway — Shareholder Letters](https://www.berkshirehathaway.com/letters/letters.html)
- [Warren Buffett Archive](https://buffett.cnbc.com/warren-buffett-archive/)
- [Charlie Munger — Poor Charlie's Almanack (Stripe Press)](https://www.stripe.press/poor-charlies-almanack)

**Essays, mental models & the bigger picture**
- [Naval Ravikant — How to Get Rich (without getting lucky)](https://nav.al/rich)
- [Paul Graham — Essays](https://www.paulgraham.com/articles.html)
- [FS Blog — Mental Models](https://fs.blog/)
- [Y Combinator — How to Start a Startup](https://www.youtube.com/watch?v=CBYhVcO4WgI&list=PL5q_lef6zVkaTY_cT1k7qFNF2TidHCe-1)
- [Sam Altman — Moore's Law for Everything](https://moores.samaltman.com/)

---

## Journals

- *Journal of Finance*
- *Journal of Financial Economics*
- *Journal of Economic Perspectives*
- *Journal of Portfolio Management*
- *Journal of Wealth Management*

---

*Readings and resources are for learning only and are not financial advice.*
